"""
Modal deployment for MVE 001: D4 State Tracking
...
Usage:
    uv run modal run --detach -m train.modal_config --config configs/cs_neg_deltanet.yaml
"""
...
N_GPUS = 1
GPU_TYPE = "T4"  # Default to T4 for MVEs, use A100/H100 for larger experiments
TIMEOUT = 28800
...
volume = Volume.from_name("mve-001-results", create_if_missing=True)

app = App("mve-001-d4-state-tracking")
...
def train_with_config(config_content: str):
    ...
    cmd = (
        ...
        + [f"--num_processes={N_GPUS}", "-m", "train.run_config", "--config", config_path]
    )
