"""
Modal deployment for MVE 039: Warp-Specialized Pingpong Chunkwise Linear RNN

Kernel microbenchmark comparing:
1. Triton baseline: Sequential load → QK^T → decay → SV pipeline
2. Triton pipelined: Software-pipelined with double-buffering and overlapped ops

Tests the core hypothesis that overlapping sequential element-wise ops
(decay masking) with GEMM operations improves throughput.
...
volume = Volume.from_name("mve-039-results", create_if_missing=True)

app = App("mve-039-pingpong-chunkwise-gla")
